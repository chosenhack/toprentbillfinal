...
import type { Payment, Customer, PaymentFrequency } from '../types';
...
const FREQUENCY_TABS = [
  { value: 'all', label: 'Tutti', icon: CardIcon },
  { value: 'monthly', label: 'Mensili', icon: Calendar },
  { value: 'quarterly', label: 'Trimestrali', icon: ClockIcon },
  { value: 'biannual', label: 'Semestrali', icon: CalendarClock },
  { value: 'annual', label: 'Annuali', icon: CalendarDays },
  { value: 'oneTime', label: 'Una Tantum', icon: Check }
] as const;

const PaymentsPage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedFrequency, setSelectedFrequency] = useState<'all' | PaymentFrequency>('all');
  ...
  const monthlyPayments = useMemo(() => {
    ...
    // Filter by frequency if not 'all'
    if (selectedFrequency !== 'all') {
      filteredPayments = filteredPayments.filter(payment => {
        const customer = customers.find(c => c.id === payment.customerId);
        return customer?.paymentFrequency === selectedFrequency;
      });
    }
  ...
  }, [payments, expectedPayments, customers, currentDate, selectedFrequency]);
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold mt-1">€{monthlyStats.totalPending.toFixed(2)}</p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="border-b border-gray-200">
          <nav className="flex space-x-4 px-6 py-3" aria-label="Tabs">
            {FREQUENCY_TABS.map(({ value, label, icon: Icon }) => (
              <button
                key={value}
                onClick={() => setSelectedFrequency(value as typeof selectedFrequency)}
              ...
              >
                ...
                <span>{label}</span>
                <span className="bg-gray-100 text-gray-600 ml-2 py-0.5 px-2.5 rounded-full text-xs">
                  {monthlyPayments.filter(p => {
                    if (value === 'all') return true;
                    return p.customer?.paymentFrequency === value;
                  }).length}
                </span>
              </button>
            ))}
